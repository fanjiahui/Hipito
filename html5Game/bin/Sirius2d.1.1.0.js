...
(function()
{
     ...
     * @param endPoint   线段终点
     * @param centerPoint  圆形中点
     * @param radius       圆形半斤
     * @returns {boolean}
     */
    ss2d.lineIntersectCircle=function(startPoint,endPoint,centerPoint,radius)
    {
        var fDis = Math.sqrt((endPoint.x - startPoint.x) * (endPoint.x - startPoint.x) + (endPoint.y - startPoint.y) * (endPoint.y - startPoint.y));

        var d=new ss2d.Point();
        d.x = (endPoint.x - startPoint.x) / fDis;
        d.y = (endPoint.y - startPoint.y) / fDis;

        var e=new ss2d.Point();
        e.x = centerPoint.x - startPoint.x;
        e.y = centerPoint.y - startPoint.y;
    ...
    /**
     * 判断两条线段是否相交
     * @param aStartPoint 线段A起点
     * @param aEndPoint 选点A终点
     * @param bStartPoint 线段B起点
     * @param bEndPoint 线段B终点
     * @returns {boolean}
     */
    ss2d.lineIntersectline=function(aStartPoint,aEndPoint,bStartPoint,bEndPoint)
    {
        var v1,v2,v3,v4;
        v1=(aEndPoint.x-aStartPoint.x)*(bEndPoint.y-aStartPoint.y) - (aEndPoint.y-aStartPoint.y)*(bEndPoint.x-aStartPoint.x);
        v2=(aEndPoint.x-aStartPoint.x)*(bStartPoint.y-aStartPoint.y) - (aEndPoint.y-aStartPoint.y)*(bStartPoint.x-aStartPoint.x);

        if(v1*v2>=0)
        return false;

        v3=(bEndPoint.x-bStartPoint.x)*(aEndPoint.y-bStartPoint.y) - (bEndPoint.y-bStartPoint.y)*(aEndPoint.x-bStartPoint.x);
        v4=(bEndPoint.x-bStartPoint.x)*(aStartPoint.y-bStartPoint.y) - (bEndPoint.y-bStartPoint.y)*(aStartPoint.x-bStartPoint.x);
        if(v3*v4>=0)
        return false;
     ...
     */
	 window.addEventListener("resize",function(){
		if(ss2d.stage&&ss2d.stage._isAutoAdjustToBrowser ){
			ss2d.stage.adjustToBrowser(ss2d.stage._AdjustType);
		}
	});
...
(function()
{
    ...
    (
        /** @lends ss2d.Matrix2D.prototype */
        {
            ...
            {
                //特殊优化
                var matrix3x3=new Float32Array(8);
                matrix3x3[0] = v1[0] * v2[0] + v1[1] * v2[3];
                matrix3x3[1] = v1[0] * v2[1] + v1[1] * v2[4];
                matrix3x3[3] = v1[3] * v2[0] + v1[4] * v2[3];
                matrix3x3[4] = v1[3] * v2[1] + v1[4] * v2[4];
                matrix3x3[6] = v1[6] * v2[0] + v1[7] * v2[3] +v2[6];
                matrix3x3[7] = v1[6] * v2[1] + v1[7] * v2[4] +v2[7];
                return matrix3x3;
            },
...
(function()
{
    ...
    (
        /** @lends ss2d.Group.prototype */
        {
            ...
            {
                var index;
                if(child instanceof ss2d.MovieClip||child instanceof ss2d.Quad)
                {
                    index=this.m_quadList.indexOf(child);
                    if(index==-1)
                    {
                        child.setTransform(this.matrix2D);
                        this.m_quadList.push(child);

                    }
                }else if(child instanceof ss2d.Group)
                {
                    index=this.childGroupList.indexOf(child);

                    if(index==-1)
                    {
                        child.parentGroup=this;
                        child.removeEvent();
                        this.childGroupList.push(child);
                    }
            ...
            removeChild:function(child)
            {
                ...
                if(child instanceof ss2d.MovieClip||child instanceof ss2d.Quad)
                {
                    ...
                    if(index!=-1)
                    {
                        ...
                        this.m_quadList.splice(index,1);
                    }
                }else if(child instanceof ss2d.Group)
                {
                    index=this.childGroupList.indexOf(child);
            ...
            upData:function()
            {
                ...
                {
                    this.matrix2D.upDateMatrix(
                        this.m_rotation,
                            (this._x)*2/ss2d.Stage2D.stageHeight,
                            (this._y)*2/ss2d.Stage2D.stageHeight,
                        this.m_scaleX,
                        this.m_scaleY,
                        this.m_skewX,
                        this.m_skewY);
                    ...
                    this.matrix2D.rawData=this.matrix2D.add3x3(this.matrix2D.rawData,this.parentGroup.matrix2D.rawData);


                    for(var i=0;i<this.m_quadList.length;i++)
                    {
                        this.m_quadList[i].startRedraw();
                    }

                    for(var i=0;i<this.childGroupList.length;i++)
                    {
                        this.childGroupList[i].upData();
                    }
                }
            ...
            dispose : function()
            {
                ...
                for(var i=0; i < this.m_quadList.length; i++)
                {
                    this.removeChild(this.m_quadList[i]);
                    this.m_quadList[i]=null;
                    //Group不负责显示层的销毁,quad由Scene负责统一销毁
                    //this.m_quadList[i].dispose();
                }

                for(var j=0;j<this.childGroupList.length;j++)
                {
                    this.removeChild(this.childGroupList[j]);
                    this.childGroupList[j]=null;
                }
    ...
    ss2d.Stats = Class(
    {
        ...
        update: function ()
        {
            ...
            if ( this._time > this._timeLastSecond + 1000 )
            {
                ...
                this._drawText.textContent = 'DRAW: ' + (ss2d.Stage2D.instance ? ss2d.Stage2D.instance.drawNumber():0);
        ...
        mCreateFps : function()
        {
            ...
            this._drawText = document.createElement( 'div' );
            this._drawText.style.fontFamily = 'Helvetica, Arial, sans-serif';
            this._drawText.style.fontSize = '9px';
            this._drawText.style.color = 'rgb(' + this._fpsColors[ 1 ][ 0 ] + ',' +
                parseInt(this._fpsColors[ 1 ][ 1 ]*1.5) + ',' + this._fpsColors[ 1 ][ 2 ] + ')';
            this._drawText.style.fontWeight = 'bold';
            this._drawText.style.padding = '0 1px';
            this._drawText.innerHTML = 'DRAW';
            this._fpsDiv.appendChild( this._drawText );

            this._fpsGraph = document.createElement( 'div' );
            this._fpsGraph.style.position = 'relative';
            this._fpsGraph.style.width = '94px';
            ...
            while ( this._fpsGraph.children.length < 94 )
            {
        ...
        mCreateMs : function()
        {
            ...
            this._msGraph.style.width = '94px';
            ...
            while ( this._msGraph.children.length < 94 )
            {
                this._bar = document.createElement( 'span' );
                this._bar.style.width = '1px';
    ...
    (
        /** @lends ss2d.DisplayObject.prototype */
        {
           ...
           getColor : function(){ return ss2d.ColorUtil.RGBToHex({
            ...
            {
                var point=this.getAABB();
                var distanceX = Math.abs(this.getAABBCoordinate().x-x);
                var distanceY = Math.abs(this.getAABBCoordinate().y-y);
            ...
            {
                var distanceX = Math.abs(this.getAABBCoordinate().x - child.getAABBCoordinate().x);
                var distanceY = Math.abs(this.getAABBCoordinate().y - child.getAABBCoordinate().y);

                if (distanceX <= this.getWidth() / 2 + child.getWidth() / 2 &&distanceY <= this.getHeight() / 2 + child.getHeight() / 2)
                {
                    return true;
                }
                return false
...
(function()
{
    ...
    (
        /** @lends ss2d.FrameBuffer.prototype */
        {
            ...
            {
                if(this._frameBuff!=null)
                this._frameBuff.isFrameBuff=false;
                this._frameBuff=value;
                this._frameBuff.isFrameBuff=true;
            },
        ...
        /** @lends ss2d.Scene.prototype */
        {
            ...
            showQuad : function(child,parameter)
            {
                if(!parameter)
                {
                    ...
                    if (index != -1)
                    {
                        //var id=this.quadList[this.quadList.length-1].m_id;
                        for(var i=0;i<this.quadList.length;i++)
                        {
                            var quad = this.quadList[i];

                                if(quad.m_id>child.m_id)
                                {
            ...
            {
                var quads = null;
                if (this.quadList == null) return null;
                for(var i = this.quadList.length - 1;i >= 0; i--)
                {
                    if(this.quadList[i].getMouseEnabled() && this.quadList[i].getScene()!= null && this.quadList[i].getVisible())
                    {
                        ...
                        {
                            var point=new ss2d.Point(
                                (x*2 / ss2d.Stage2D.stageHeight),
                                (y*2 / ss2d.Stage2D.stageHeight));


                            var a=this.quadList[i].getVertex(0);
                            a.x+=((this.quadList[i].GPUX)*2/ ss2d.Stage2D.stageHeight);
                            a.y+=((this.quadList[i].GPUY)*2/ ss2d.Stage2D.stageHeight);


                            var b=this.quadList[i].getVertex(1);
                            b.x+=((this.quadList[i].GPUX)*2/ ss2d.Stage2D.stageHeight);
                            b.y+=((this.quadList[i].GPUY)*2/ ss2d.Stage2D.stageHeight);


                            var c=this.quadList[i].getVertex(2);
                            c.x+=((this.quadList[i].GPUX)*2/ ss2d.Stage2D.stageHeight);
                            c.y+=((this.quadList[i].GPUY)*2/ ss2d.Stage2D.stageHeight);


                            var d=this.quadList[i].getVertex(3);
                            d.x+=((this.quadList[i].GPUX)*2/ss2d.Stage2D.stageHeight);
                            d.y+=((this.quadList[i].GPUY)*2/ ss2d.Stage2D.stageHeight);
                    ...
                    if(thisQuad.scene != null && thisQuad.m_visible && thisQuad.m_isActivate)
                    {
                        ...
                        if(thisQuad.isRedraw)
                        {
                            ...
                            if(this.isFrameBuff)
                            {
                                ...
                                thisQuad.m_quadMatrixUtil.setY(thisQuad.y-(ss2d.Stage2D.stageHeight-thisQuad.getHeight()) * 2 /ss2d.Stage2D.stageHeight);
                            }else
                            {
                                thisQuad.m_quadMatrixUtil.setX(thisQuad.x * 2 / ss2d.Stage2D.stageHeight);
                        ...
                        {
                            thisQuad.verticesColor[thisQuad.id16_1]  = 1;
                            thisQuad.verticesColor[thisQuad.id16_2]  = 1;
                            thisQuad.verticesColor[thisQuad.id16_3]  = 1;
                            thisQuad.verticesColor[thisQuad.id16_4]  = 1;

                            thisQuad.verticesColor[thisQuad.id16_5]  = 1;
                            thisQuad.verticesColor[thisQuad.id16_6]  = 1;
                            thisQuad.verticesColor[thisQuad.id16_7]  = 1;
                            thisQuad.verticesColor[thisQuad.id16_8]  = 1;

                            thisQuad.verticesColor[thisQuad.id16_9]  = 1;
                            thisQuad.verticesColor[thisQuad.id16_10] = 1;
                            thisQuad.verticesColor[thisQuad.id16_11] = 1;
                            thisQuad.verticesColor[thisQuad.id16_12] = 1;

                            thisQuad.verticesColor[thisQuad.id16_13] = 1;
                            thisQuad.verticesColor[thisQuad.id16_14] = 1;
                            thisQuad.verticesColor[thisQuad.id16_15] = 1;
                            thisQuad.verticesColor[thisQuad.id16_16] = 1;
                        }
        ...
        /** @lends ss2d.Sprite.prototype */
        {
            ...
            addChild:function(child)
            {
                ...
                if(child instanceof ss2d.Sprite)
                {
                    ...
                    this.group.addChild(child.group)
            ...
            removeChild:function(child)
            {
                ...
                this.group.removeChild(child.m_quad);
...
(function()
{
    ...
    (
        /** @lends ss2d.MovieClip.prototype */
        {
            ...
            {
                return this.m_quad.getTileName();
            },
             ...
             * 添加帧脚本
             * <br />add frame script
             * @param FrameFunction 帧脚本类
             */
            addFrameScript : function(frameFunction)
            {
                this.m_quad.addFrameScript(frameFunction);
            },
            ...
            getAABB:function()
            {
                return this.m_quad.getAABB();
            },

            /**
             * 启用刷新
             */
            startRedraw:function()
            {
                this.m_quad.startRedraw();
            },
             ...
             * <br />get the absolute center point
             */
            getAABBCoordinate:function()
            {
                return this.m_quad.getAABBCoordinate();
            },
    ...
    (
        /** @lends ss2d.Quad.prototype */
        {
            ...
            initialize : function(id, texture, vertices, textureUV, verticesColor, jointList, indexList,verticesGPUList)
            {
                ...
                this.m_aabb=new ss2d.Point();
                this.m_aabb_coordinate=new ss2d.Point();
                this.frameList=[];
            ...
            getVertex : function(index)
            {
                ...
                {
                    case 0:
                        return new ss2d.Point(this.leftUpPoint.x,this.leftUpPoint.y);
                    case 1:
                        return new ss2d.Point(this.leftDownPoint.x,this.leftDownPoint.y);
                    case 2:
                        return new ss2d.Point(this.rightUpPoint.x,this.rightUpPoint.y);
                    case 3:
                        return new ss2d.Point(this.rightDownPoint.x,this.rightDownPoint.y);
                }
            },
            ...
            gotoAndPlay : function(value)
            {
                if(this.m_Texture.quadResource)
                if(value<=this.m_Texture.quadResource.quadDataList[this.m_tileId].quadFrameLst.length)
                {
            ...
            addFrameScript : function(frameFunction)
            {
                this.frameList.push(frameFunction);
            },
            ...
            getAABB:function()
            {
                ...
                var absX=(maxX-minX)/2.0*ss2d.Stage2D.stageHeight
                var absY=(maxY-minY)/2.0*ss2d.Stage2D.stageHeight;
            ...
            getAABBCoordinate:function()
            {
                ...
                var ax=minX/2.0*ss2d.Stage2D.stageHeight+absX/2
                var ay=minY/2.0*ss2d.Stage2D.stageHeight+absY/2;

                this.m_aabb_coordinate.x=ax+this.GPUX;
                this.m_aabb_coordinate.y=ay+this.GPUY;

                return this.m_aabb_coordinate;
            },
            ...
            upData : function()
            {
                    //强制转换成笛卡尔第四象限坐标系
                    //transform into Cartesian coordinates (fourth quadrant)
                    this.m_quadMatrixUtil.setX(this.x * 2 / ss2d.Stage2D.stageHeight);
                    this.m_quadMatrixUtil.setY(this.y * 2 /ss2d.Stage2D.stageHeight);

                    this.m_quadMatrixUtil.setRotation(this.m_rotation);

                    this.m_quadMatrixUtil.upDateMatrixData(
                            (-this.m_pivotX + this.m_framePivotX) * this.m_scaleX * 2 / ss2d.Stage2D.stageHeight,
                            (-this.m_pivotY + this.m_framePivotY) * this.m_scaleY * 2 / ss2d.Stage2D.stageHeight,
                            this.m_scaleX * (this.m_frameWidth / this.m_Texture.width),
                            this.m_scaleY * (this.m_frameHeight / this.m_Texture.height),
                        this.m_skewX, this.m_skewY);

                    //更新面板的矩阵信息
                    //update matrix of the panel
                    this.m_quadMatrixUtil.upDateMatrix(this.m_transform == null);

                    if(this.m_transform != null)
                    {
                        this.m_quadMatrixUtil.getMatrix2D().rawData = this.m_quadMatrixUtil.getMatrix2D().add3x3(this.m_quadMatrixUtil.getMatrix2D().rawData, this.m_transform.rawData);
                        this.m_quadMatrixUtil.upDateRaw();
                    }

                    this.frameXDivWidth = this.m_frameX / this.m_Texture.width;
                    this.frameYDivHeight = this.m_frameY / this.m_Texture.height;
                    this.frameWidthDivWidth =this.m_frameWidth / this.m_Texture.width;
                    this.frameHeightDivHeight = this.m_frameHeight / this.m_Texture.height;
        ...
        (
            /** @lends ss2d.Stage2D.prototype */
            {
                ...
                initialize : function(canvasId, width, height)
                {
                    ...
                    {
                        var cv = ss2d.$new("canvas");
                        document.body.appendChild(cv);
                        ss2d.Stage2D.canvas = cv;
                    }
                    width = width || "auto";
                    height = height || "auto";
                 ...
                 * @param colorArray 像素的缓存数组
                 * @param x 从X点开始读取像素信息
                 * @param y 从Y点开始读取像素信息
                 * @param width 读取像素区域的宽度
                 * @param height 读取像素区域的高度
                 */
                readPixels:function(colorArray,x,y,width,height,readPixelsCallBback)
                {
                    this.readPixelsX=x;
                    this.readPixelsY=y;
                    this.readPixelsWidth=width;
                    this.readPixelsHeight=height;
                    this.colorReadPixels=colorArray;
                    this.readPixelsCallBback=readPixelsCallBback;
                },
                ...
                {
                    ss2d.stageScaleX=scaleX;
                    ss2d.stageScaleY=scaleY;
                    ss2d.stage.size(this.cacheWidth,this.cacheHeight);
                    ss2d.Stage2D.gl.viewport(0, 0, ss2d.Stage2D.canvas.width, ss2d.Stage2D.canvas.height);
                },
                ...
                size : function (width, height)
                {
                    ...
                    ss2d.Stage2D.canvas.style.width = (width*ss2d.stageScaleX) + "px";
                    ss2d.Stage2D.canvas.style.height = (height*ss2d.stageScaleY) + "px";
				...
				adjustToBrowser:function(AdjustType){
					var gW=this.cacheWidth,
						gH=this.cacheHeight
						sW=ss2d.getScreenSize().width,
						sH=ss2d.getScreenSize().height,
						gameRatio=gW/gH,
						screenRatio=sW/sH,
						scaleX=1,scaleY=1;
						if (this._stats){
					 		this._stats.getDomElement().style.left = ss2d.getElementOffset(ss2d.Stage2D.canvas).left + 'px';
				            this._stats.getDomElement().style.top = ss2d.getElementOffset(ss2d.Stage2D.canvas).top + 'px';
					 	}
						if(this._AdjustType!=AdjustType){
							this._AdjustType=AdjustType
						}
					switch(AdjustType){
						case ss2d.screenAdjust.Ratio:
						...
						break;
						case ss2d.screenAdjust.Fill:
							scaleX=sW/gW;
							scaleY=sH/gH;
						break;
						case ss2d.screenAdjust.Custom:
							if(this.CustomAutoAdjustToBrowser){
								var s={ Width:ss2d.getScreenSize().width,
										Height:ss2d.getScreenSize().height},
									o={	Width:ss2d.stage.cacheWidth,
										Height:ss2d.stage.cacheHeight};
								ss2d.stage.CustomAutoAdjustToBrowser.apply(ss2d.stage,[{Screen:s,Original:o}]);
							}
						break;
						default:	
							if(screenRatio<gameRatio){
					...
					if(AdjustType!=ss2d.screenAdjust.Custom){
                ...
                drawNumber:function()
                {
                    return this.displayerlist.length+this.frameBufflist.length;
                },
                ...
                {
                    var index=this.frameBufflist.indexOf(value);
                    if(index==-1)
                 ...
                 * @param value
                 */
                removeFrameBuffer:function(value)
                {
                    var index=this.frameBufflist.indexOf(value);
                    if(index!=-1)
                    this.frameBufflist.splice(index,1)
                },
                ...
                onDrawFrame : function()
                {
                    ...
                    if(this.colorReadPixels!=null)
                    {
                        ss2d.Stage2D.gl.readPixels(
                            this.readPixelsX,
                            ss2d.Stage2D.stageHeight-this.readPixelsY,
                            this.readPixelsWidth,
                            this.readPixelsHeight,
                            ss2d.Stage2D.gl.RGBA,
                            ss2d.Stage2D.gl.UNSIGNED_BYTE,
                            this.colorReadPixels
                        );
                        if(this.readPixelsCallBback)
                            this.readPixelsCallBback(this.colorReadPixels);
                        this.readPixelsCallBback=null;
                    }
                ...
                onMouseEventHandler : function(e)
                {
                    ...
                    ss2d.Stage2D.mouseX = (this.tx - pos.left)/ss2d.stageScaleX
                    ss2d.Stage2D.mouseY = (this.ty - pos.top)/ss2d.stageScaleY;


                    var evt = new ss2d.MouseEvent(type, false, false, ss2d.Stage2D.mouseX, ss2d.Stage2D.mouseY);

                    for(var i=this.frameBufflist.length-1;i>=0;i--)
                    {
                        for(var j=this.frameBufflist[i]._sceneList.length-1;j>=0;j--)
                        {
                            var scene = this.frameBufflist[i]._sceneList[j];
                            //是否开启鼠标事件
                    ...
                    if (type == ss2d.MouseEvent.MOUSE_MOVE)
                    {
                        if (this.moveTarget)
                        {
                            if (hitObj == null || hitObj != this.moveTarget )
                            {
                                this.moveTarget.dispatchEvent(new ss2d.MouseEvent(ss2d.MouseEvent.MOUSE_OUT, false, false, ss2d.Stage2D.mouseX, ss2d.Stage2D.mouseY));
                            }
                        }
                        if (hitObj &&  hitObj != this.moveTarget)
                        {
                            hitObj.dispatchEvent(new ss2d.MouseEvent(ss2d.MouseEvent.MOUSE_OVER, false, false, ss2d.Stage2D.mouseX, ss2d.Stage2D.mouseY));
                        }
                        this.moveTarget = hitObj;
                    }
        ...
        /** @lends ss2d.Texture.prototype */
        {
            ...
            initialize : function(value)
            {
                ...
                }else if(arguments.length==3)
                {
                    ...
                    }else if(!isNaN(arguments[0])&&!isNaN(arguments[1])&&arguments[2] instanceof Uint8Array)
                    {
                        this.width = arguments[0];
                        this.height = arguments[1];
                        this.newBitmapData(arguments[2],this.width,this.height,0x2901,0x2901,0x2601,0x2601);
                    }else
                    {
                        new ss2d.Error(1005);
                    }
                ...
                {
                    if(!isNaN(arguments[0])&&!isNaN(arguments[1])&&arguments[2] instanceof Uint8Array&&arguments[3] instanceof ss2d.TextureStyle)
                    {
                        this.width = arguments[0];
                        this.height = arguments[1];
                        this.newBitmapData(arguments[2],this.width,this.height,arguments[3].xTile,arguments[3].yTile,arguments[3].xSampling,arguments[3].ySampling);
                    }
                }else
                {
                    new ss2d.Error(1005);
             ...
             * @param ySampling
             */
            newBitmapData:function(array,width,height,xTile,yTile,xSampling,ySampling)
            {
                //清空状态机里的纹理,这里只是清除引用而已,不是清除纹理,纹理我们已经经过状态机加工过了
                //only delete the ref
                this.texture = ss2d.Stage2D.gl.createTexture();
                ...
                ss2d.Stage2D.gl.texImage2D(ss2d.Stage2D.gl.TEXTURE_2D, 0,ss2d.Stage2D.gl.RGBA,width,height,0, ss2d.Stage2D.gl.RGBA, ss2d.Stage2D.gl.UNSIGNED_BYTE,array);
        ...
        /** @lends ss2d.ShaderGray.prototype */
        {
            ...
            initialize : function()
            {
                ...
                this.fragmentShader = this.getShader(ss2d.Stage2D.gl,
                        ...
                        "gl_FragColor=gl_FragColor*alphaValue;"+
                        "}","fs");
            ...
            upDataShader : function(scene)
            {
                ...
                this.upDataVertex(scene);
                this.upDataCache(scene);
                ss2d.Stage2D.gl.uniform4f(ss2d.Stage2D.gl.getUniformLocation(this.shaderProgram,"rgbaValue"),this.r,this.g,this.b,this.a);
                ss2d.Stage2D.gl.uniform1f(ss2d.Stage2D.gl.getUniformLocation(this.shaderProgram,"alphaValue"),this.alpha);
            }
        }
    );
})();
        ...
        /** @lends ss2d.ShaderLight.prototype */
        {
            ...
            initialize : function()
            {
                ...
                this.fragmentShader = this.getShader(ss2d.Stage2D.gl,
                        ...
                        "lighting.x=abs(lightingValue.x-vTextureUV.x)*scaleValue.x;"+
                        "lighting.y=abs(lightingValue.y-vTextureUV.y)*scaleValue.y;"+
            ...
            upDataShader : function(scene)
            {
                ...
                ss2d.Stage2D.gl.uniform2f(ss2d.Stage2D.gl.getUniformLocation(this.shaderProgram,"scaleValue"),this.scaleX,this.scaleY);
            }
        }
    );
})();
