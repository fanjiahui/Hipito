        ...
        /** @lends demo.Main.prototype */
        {
           ...
           _handleOverallProgress : function(e)
            ...
            _init:function()
            {
               ...
               //创建飞机爆炸纹理
                this.planeBlastScene=new ss2d.Scene(new ss2d.Texture(demo.Main.assets.getResult("explode.png"),demo.Main.assets.getResult("explode.xml")),50);
                ss2d.stage.addChild(this.planeBlastScene);
                //创建文本
                this.text1=new ss2d.TextField(512,64);
                this.text1.setFontSize(50);// 字体大小
                this.text1.setColor(.1,.8,.2); //rgb  0~1 即n/255
                this.text1.setText("击毁 "+this.bitPlane+" 架!");
                this.text1.setX(40);
                this.text1.setY(10);
                ss2d.stage.addChild(this.text1);
            ...
            onEnterFrameHandler : function(e)
            {
                ...
                for(var j=0;j<this.enemyList.length;j++)
                {
                    ...
                    if(this.enemyList[j].hitTestRoundness(this.bulletList[n].GPUX,this.bulletList[n].GPUY,30))
                    {
                        ...
                        //如果血量少于0，播放爆炸动画
                        if(this.enemyList[j].getUserData()[0],this.enemyList[j].getUserData()[1]<=0){
                            //同时创建敌人爆炸 pcj
                            var blst=this.planeBlastScene.applyQuad();
                            if(blst!=null)
                            {
                                blst.setTileName("explode");
                                blst.setAnimationSpeed(24);
                                blst.loop(false);
                                blst.GPU=true;
                                blst.GPUX=this.enemyList[j].GPUX;
                                blst.GPUY=this.enemyList[j].GPUY;
                                this.planeBlastScene.showQuad(blst,true);
                                this.blastList.push(blst);
                               
                            }
                            this.bitPlane++;//击落个数加一 pcj
                            this.text1.setText("击毁 "+this.bitPlane+" 架!");
                        }
                        break;
                    }
                ...
                //遍历爆炸 pcj
                for(var bz=0;bz<this.blastList.length;bz++)
                {
                    if(this.blastList[bz].getCurrentFrame()==12){//动画播放完 pcj
                         //隐藏爆炸
                        this.planeBlastScene.hideQuad(this.blastList[bz]);
                    }
                }
